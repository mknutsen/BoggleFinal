...
/**
 * Given a Boggle object, it finds all of the words (according the mknutsen.boggle.dictionary in the Boggle object) that
 * exist on the boggle board
 *
...
public class TraverseBoggleBoard {
    ...
    /**
     * Simple test. Finds all the words in the board, copies it to an array and then sorts it and prints it so i could
     ...
     *
     * @return an arraylist of arraylists of boggletiles. this will make it easy to construct a trajectory and display
     * the found words on the boggle board (each internal arraylist represents a word)
     */
    public ArrayList<ArrayList<BoggleTile>> analyze() {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                ...
                //				word.addAll(checkPath("", board[i][j], new Hashtable<BoggleTile, Integer>(), new
                // ArrayList<String>()));
    ...
    /**
     * This is a recursive method that only continues if the node for the character in the trie has children (so if
     * there might be words that follow, it continues looking)
     *
     * @param word
     ...
     */
    private ArrayList<ArrayList<BoggleTile>> checkPath(ArrayList<BoggleTile> word,
                                                       ArrayList<ArrayList<BoggleTile>> foundWordPaths,
    ...
    /**
     * Wrapper class for the boggle board so each tile contains its character and the coordinates at which it is
     * located
     *
     * @author Max
     */
    public class BoggleTile {
        ...
        /**
         * I dont think this is needed, but the hashCode is the ascii value for the character concatonated with the x
         * and y values
         */
        public int hashCode() {
            int ascii = c;
