/**
I would set up the graph by first creating a 4x4 array of BoggleTiles. These would each hold a character and a hash table.
The hash table would have characters as keys and array lists as values.
The value array list would be the references to each neighbor with that letter.
In this way we can easily look at a tile, decide what letters are around it, and jump to the next tile.
Encapsulating the boggle tile array should be a boggle graph object. This should contain a method asking isWordInGtaph.
The graph object should also have a hash with letters mapped to array lists of boggle tiles. This will be useful to get the search started. 
Then, I'll you should have to do is:
In the graph:
Pop off the first letter
If letter is in hash
    For each tile in array list
        If ( canMakeWordUsingTile(tile, rest of word)
            Return true
Return false

Then your tiles should do a similar form of recursion where they pop off the first letter, see if it's in the tile's hash,
then send the rest of the word to EACH of its neighbors with that letter and repeat!
...
public class BoggleGraphBrute extends BoggleGraph{
	...
	/* (non-Javadoc)
	 * @see mknutsen.boggle.bogglegraph.BoggleGraph#next(mknutsen.boggle.bogglegraph.BoggleTile, java.lang.String, java.util.Hashtable)
	 */
	@Override
